  ...
  <div class = "main_catalog">
        ...
        <%
           ...
           try {
              ...
              if (orderby == null)
                 phones = dao.selectListcode(conn,pcode);
              else if (orderby.equals("pdate"))
                 phones = dao.listByPdatecode(conn,pcode);
              else
                 phones = dao.listByPcountcode(conn,pcode);   
...
function putcart(frm){
   var amount = frm.amount.value.trim();
   var id = frm.pid.value.trim(); 
   window.open("putCart.jsp?pid="+id+"&amount="+amount,"","width=1000, height=300");
}
